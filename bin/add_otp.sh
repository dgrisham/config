...
[[ -z "$otp_url" ]] && { notify-send "failed to extract otp qr code from screenshot" ; exit 1 ; }
...
if echo "$otp_url" | pass otp append "$entry"; then
    notify-send "Successfully added OTP URL for $entry"
else
    notify-send "Failed to add OTP URL for $entry"
fi
