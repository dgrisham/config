...
_linode_cli()
{
    ...
    case "${prev}" in
        linode-cli)
            COMPREPLY=( $(compgen -W "account events payment-methods service-transfers users databases domains images linodes kernels stackscripts lke longview managed networking firewalls vlans nodebalancers object-storage profile sshkeys regions tickets tags volumes --help" -- ${cur}) )
            return 0
            ;;
        account)
        ...
        users)
            ...
            ;;
        databases)
            COMPREPLY=( $(compgen -W "engines engine-view list mysql-list mysql-create mysql-view mysql-update mysql-delete mysql-backups-list mysql-backup-view mysql-backup-restore mysql-creds-view mysql-creds-reset mysql-ssl-cert types type-view --help" -- ${cur}) )
            return 0
            ;;
        domains)
